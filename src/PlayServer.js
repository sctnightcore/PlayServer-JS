...
async function saveimg(checksum) {
	try {
		...
		console.log(colors.grey(`[GetImg/SaveImg]:${checksum.data.checksum}.png`));
	} catch (err) {
		console.error(err);
	}	
}
...
async function sendanswer(checksum,ans) {
	try {
		...
		if (json.data.success == true) {
			console.log(colors.green(`[Success]:${checksum.data.checksum}.png is ${ans}`));
		} else {
			console.log(colors.red(`[Fail]:${checksum.data.checksum}.png isn't ${ans}`));
		}
		return json;
	} catch (err) {
		console.error(err);
